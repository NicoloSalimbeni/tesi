...
ObjAnEs::ObjAnEs()
{
    Double_t res_cos_min = -0.2, res_cos_max = 0.2;        // tlb_Btag.M():-0.004 0.004 vertex -0.2 0.2   mass_B: -0.5 0.5;
    Double_t res_corr_min = -0.2, res_corr_max = 0.2;      // tlb_Btag.M(): -0.004 0.004 vertex: -0.2 0.2     mass_B: -0.1 0.1;
    Double_t res_mean_min = -1, res_mena_max = 1;          // tlb_Btag.M():-1 1 = vertex      mass_B: -0.1 0.8;
    Double_t res_collcomp_min = -1, res_coll_comp_max = 1; // tlb_Btag.M():-0.005 0.005 vertex -1 1      mass_B: -0.5 0.5;
    Double_t res_rand_min = -0.2, res_rand_max = 0.2;      // tlb_Btag.M():-0.004 0.004 vertax: -0.2 0.2      mass_B: ;

    Double_t inv_mass_min = 1;
    Double_t inv_mass_max = 4.5;

    AnalysisSteering::subscribe(this);

    // tlb_Btag.M():-0.004 0      mass_B: -0.3 0.1;
    Resolution_an_mag = new TH2D("Resolution_anEs_mag", "risoluzione vs massa visibile,  metodo analitico soluzione maggiore", 100, inv_mass_min, inv_mass_max, 300, -0.004, 0);
    ...
    Resolution_an_min = new TH2D("Resolution_anEs_min", "risoluzione vs massa visibile,  metodo analitico soluzione minore", 100, inv_mass_min, inv_mass_max, 300, 0, 0.01);
    ...
    // tlb_Btag.M(): -0.004 0.004 vertex:      mass_B: -0.1 0.1;
    Resolution_an_corr = new TH2D("Resolution_anEs_corr", "risoluzione vs massa visibile,  risultati esatti", 100, inv_mass_min, inv_mass_max, 300, res_corr_min, res_corr_max);
    ...
    Resolution_an_mean = new TH2D("Resolution_anEs_mean", "risoluzione vs massa visibile,  media soluzioni analitiche", 100, inv_mass_min, inv_mass_max, 300, res_mean_min, res_mena_max);
    ...
    an_mean_profile = new TProfile("Profile_anEs_mean", "Profilo Massa vs risoluzione, media soluzioni", 25, inv_mass_min, inv_mass_max, -3, 3);
    ...
    // tlb_Btag.M():-0.004 0.004 vertex -0.2 0.2   mass_B: -0.5 0.5;
    Resolution_an_cos = new TH2D("Resolution_anEs_cos", "risoluzione vs massa visibile,  cos#theta maggiore", 100, inv_mass_min, inv_mass_max, 300, res_cos_min, res_cos_max);
    ...
    an_cos_profile = new TProfile("Profile_anEs_cos", "Profilo Massa vs risoluzione,  cos#theta maggiore", 25, inv_mass_min, inv_mass_max, -3, 3);
    ...
    Resolution_an_collcomp = new TH2D("Resolution_anEs_collcomp", "risoluzione vs massa visibile,  coll comp", 100, inv_mass_min, inv_mass_max, 300, res_collcomp_min, res_coll_comp_max);
    ...
    an_collcomp_profile = new TProfile("Profile_anEs_collcomp", "Profilo Massa vs risoluzione,  collcomp", 25, inv_mass_min, inv_mass_max, -3, 3);
    ...
    // tlb_Btag.M():-0.004 0.004 vertax: -0.2 0.2      mass_B: ;
    Resolution_an_random = new TH2D("Resolution_anEs_random", "risoluzione vs massa visibile,  random", 100, inv_mass_min, inv_mass_max, 300, res_rand_min, res_rand_max);
    ...
    an_random_profile = new TProfile("Profile_anEs_random", "Profilo Massa vs risoluzione,  random", 25, inv_mass_min, inv_mass_max, -3, 3);
    ...
    f_fit_random = new TF1("f_anEs_random", "pol4", 1, 4);
    ...
    nbin = 100;
    start = -1, stop = 1; // tlb_Btag.M(): -0.01 0.01 vertex -1 1     mass_B: -0.06 1;
    binwidth = (stop - start) / nbin;
    s_binwidth = std::to_string(binwidth);
    s_binwidth.resize(5);
    ...
    nbin = 100;
    start = -0.6, stop = 0.6; // tlb_Btag.M(): -0.01 0.01  vertex -0.6 0.6    mass_B: -0.04 0.04;
    binwidth = (stop - start) / nbin;
    s_binwidth = std::to_string(binwidth);
    s_binwidth.resize(5);
    ...
    nbin = 100;
    start = -1.2, stop = 1.2; // tlb_Btag.M():-0.05 0.005 vertex -1.2 1.2      mass_B: -0.2 0.2;
    binwidth = (stop - start) / nbin;
    s_binwidth = std::to_string(binwidth);
    s_binwidth.resize(5);
    ...
    nbin = 100;
    start = -1, stop = 1; // tlb_Btag.M():-0.3 0.3 vertex -0.6 0.6      mass_B:;
    binwidth = (stop - start) / nbin;
    s_binwidth = std::to_string(binwidth);
    s_binwidth.resize(5);
    h_residui_random = new TH1D("Hist_residui_AnEs_random", "residui percentuali analitica random (E_{vera}-E_{stimata})/E_{vara}", nbin, start, stop);
...
{
    Double_t pvz = (tlv_visibile.Vect().Dot(tlv_vertex.Vect())) / tlv_vertex.Vect().Mag();
