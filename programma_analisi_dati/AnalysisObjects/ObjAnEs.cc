...
ObjAnEs::ObjAnEs()
{
    ...
    // tlb_Btag.M():      mass_B: ;
    Resolution_an_random = new TH2D("Resolution_anEs_random", "risoluzione vs massa visibile,  random", 100, 1, 4.5, 300, -0.004, 0.004);
    ...
    an_random_profile = new TProfile("Profile_anEs_random", "Profilo Massa vs risoluzione,  random", 25, 1, 4.5, -3, 3);
    ...
    nbin = 100;
    start = -0.3, stop = 0.3; // tlb_Btag.M():      mass_B:;
    binwidth = (stop - start) / nbin;
    s_binwidth = std::to_string(binwidth);
    s_binwidth.resize(5);
